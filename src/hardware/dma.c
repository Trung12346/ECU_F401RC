...
void dma2_init(uintptr_t adc_mbuf, uintptr_t tim_mbuf)
{
	...
	DMA2_Stream1->CR &= ~(uint32_t)
	(
		DMA_SxCR_CHSEL | //channel6 tim1_ch1
		DMA_SxCR_DIR     //peripheral to memory
	);
	...
	DMA2_Stream1->NDTR = (uint32_t)0x1U;
	
	DMA2_Stream0->PAR = (uintptr_t)&ADC1->DR;
	DMA2_Stream1->PAR = (uintptr_t)&TIM2->CNT;
	
	DMA2_Stream0->M0AR = (uintptr_t)adc_mbuf;
	DMA2_Stream1->M0AR = (uintptr_t)tim_mbuf;
}
